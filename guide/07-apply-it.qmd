...
The areal interpolation logic in the Python pipeline mirrors the `sdc-redistribute` family of functions available in R. An R implementation of the full Arlington pipeline is forthcoming, using `tigris` for Census geometries, `censusapi` or `tidycensus` for ACS data, `ooklaOpenDataR` for speed-test tiles, and `areal` or a direct port of `redistribute_direct` for the interpolation step. Analysts already working in R can follow the same six-step recipe using those packages; the redistribution algebra is language-agnostic.
